...
export default function ScheduleLeftBottomBox({ selectedDate }) {
  ...
  React.useEffect(() => {
    if (!selectedDate || !Array.isArray(allDepartments) || allDepartments.length === 0) {
      setMeetings([]);
      return;
    }
  ...
  return (
    <div className="w-full h-full bg-white rounded-2xl shadow-xl border border-gray-100 p-4 flex flex-col transition-all duration-300 hover:shadow-2xl dark:bg-gray-800 dark:border-gray-700">
      <h3 className="text-lg font-bold text-gray-800 mb-3 flex items-center gap-2 dark:text-gray-100">
        ...
        {selectedDate ? `${formatDate(selectedDate)}'s Meetings` : "Today's Meetings"}
      </h3>
      <div className="flex-1 flex flex-col gap-3 overflow-y-auto">
        {loading ? (
          ...
          meetings.map((meeting) => (
            <div key={meeting.id} className={`flex items-center gap-3 p-3 rounded-xl border border-gray-100 shadow-sm ${meeting.color} transition-all duration-200 hover:scale-[1.02] dark:border-gray-600`}> 
              <div>{meeting.icon}</div>
              <div className="flex flex-col flex-1 min-w-0">
                <span className="font-semibold text-gray-800 text-sm truncate dark:text-gray-100">{meeting.title}</span>
                <span className="text-xs text-gray-500 font-medium dark:text-gray-400">{meeting.time}</span>
                <span className="text-xs text-gray-600 font-medium dark:text-gray-300">Assignee: {meeting.assignee}</span>
          ...
          <div className="text-gray-400 text-sm text-center mt-6 dark:text-gray-500">
            {selectedDate ? `No meetings on ${formatDate(selectedDate)}` : 'No meetings today'}
          </div>
        )}
      </div>
    </div>
