...
const [editedDescription, setEditedDescription] = React.useState(selectedMeeting?.description || "");
  ...
  const getDepartmentName = (departmentId) => {
    ...
    if (!departmentId || !Array.isArray(allDepartments)) {
      console.log("Returning N/A because:", {
        departmentId,
        allDepartmentsLength: allDepartments?.length,
      });
      return "N/A";
  ...
  const containerWidth =
    maxOverlapping > 1
      ? maxOverlapping * (MEETING_BOX_WIDTH + MEETING_BOX_GAP) + "px"
      : "100%";
  ...
  const startEdit = () => {
    ...
    const editDataToSet = {
      ...
      start_time: selectedMeeting.start_time
        ? selectedMeeting.start_time.substring(0, 5)
        : selectedMeeting.start?.split(" ")[0],
  ...
  const handleEditMeeting = async () => {
    ...
    if (editData.date === todayStr) {
      ...
      if (
        startEastern.isBefore(nowEastern) ||
        startEastern.isSame(nowEastern)
      ) {
    ...
    try {
      ...
      console.log(
        "All departments available during edit mapping:",
        allDepartments
      );
      const mappedMeetings = data
        .filter((meeting) => {
        ...
        .map((meeting) => {
          ...
          console.log(
            "Meeting department field during edit:",
            meeting.department
          );
 ...
 const handleMeetingDescription = async (newDescription) => {
  ...
  const handleAddMeeting = async (e) => {
    ...
    try {
      ...
      console.log(
        "All departments available during add mapping:",
        allDepartments
      );
      const mappedMeetings = data
        .filter((meeting) => {
        ...
        .map((meeting) => {
          ...
          console.log(
            "Meeting department field during add:",
            meeting.department
          );
              ...
              <div className="flex flex-col">
                ...
                {TIME_SLOTS.map((slot, idx) => {
                  ...
                  return (
                    <div
                      key={slot}
                      className="h-12 flex items-center justify-end pr-2 text-xs text-gray-400 dark:text-gray-500"
                    >
                      <div className="text-right">
              ...
              <div className="relative overflow-x-auto bg-time-grid">
                <div style={{ width: containerWidth, minWidth: "100%" }}>
                  ...
                  {meetings.map((meeting) => {
                    ...
                    const height =
                      (getSlotIndex(meeting.end) -
                        getSlotIndex(meeting.start) +
                        1) *
                      48;
                    const { left, width, totalOverlapping } =
                      calculateMeetingLayout(meeting, meetings);

                    // Convert times to EST for display
                    const estStart = convertToPakistanTime(
                      meeting.start,
                      meeting.date
                    );
                    const estEnd = convertToPakistanTime(
                      meeting.end,
                      meeting.date
                    );
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-white/70 dark:bg-gray-700/70 rounded-xl p-3 text-center">
                      ...
                      <div className="text-base font-bold text-gray-800 dark:text-gray-200">
                        {mounted
                          ? new Date(
                              `${selectedMeeting.date}T${selectedMeeting.start_time}`
                            ).toLocaleTimeString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                        ...
                        {mounted
                          ? new Date(
                              `${selectedMeeting.date}T${selectedMeeting.end_time}`
                            ).toLocaleTimeString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                ...
                <div className="bg-gradient-to-br from-green-100 to-emerald-200 dark:from-green-900/30 dark:to-emerald-800/30 rounded-2xl p-4 border border-green-200/50 dark:border-green-700/50 shadow-lg">
                  ...
                  <div className="space-y-3">
                    <div className="bg-white/70 dark:bg-gray-700/70 rounded-xl p-3">
                      ...
                      <div className="text-base font-bold text-gray-800 dark:text-gray-200">
                        {selectedMeeting.meeting_type || "N/A"}
                      </div>
                    </div>
                    ...
                    <div className="bg-white/70 dark:bg-gray-700/70 rounded-xl p-3">
                      ...
                      <div className="text-base font-bold text-gray-800 dark:text-gray-200">
                        {selectedMeeting.department_name || "N/A"}
                      </div>
                    </div>
                        ...
                        {selectedMeeting.cc_members?.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {selectedMeeting.cc_members.map((m, index) => (
                              <span
                                key={m.id}
                                className="inline-flex items-center gap-1 bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 rounded-full px-2 py-1 text-xs font-medium dark:from-pink-900/30 dark:to-purple-900/30 dark:text-pink-300"
                              >
              ...
              <DialogDescription>
                ...
                <div className="flex gap-4 mb-2">
                  ...
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleEditChange("department", Number(e.target.value))
                      }
                      required
                ...
                <div className="mb-2">
                  ...
                  >
                    ...
                    {editData.assignee &&
                      !allUsers.some((u) => u.id === editData.assignee.id) && (
                        <option
                          key={editData.assignee.id}
                          value={editData.assignee.id}
                        >
                    ...
                    {allUsers
                      .filter(
                        (u) => !editData.cc_members?.some((m) => m.id === u.id)
                      )
                      .map((u) => (
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {editData.cc_members?.map((m) => (
                      <span
                        key={m.id}
                        className="flex items-center gap-1 bg-blue-100 text-blue-800 rounded-full px-3 py-1 text-xs font-medium shadow-sm dark:bg-blue-900/30 dark:text-blue-300"
                      >
                        {m.name}
                        <button
                          type="button"
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (
                        user &&
                        !editData.cc_members?.some((m) => m.id === user.id) &&
                        user.id !== editData.assignee?.id
                      ) {
                        handleEditChange("cc_members", [
                          ...(editData.cc_members || []),
                          user,
                        ]);
                      }
                ...
                <div className="flex gap-4 mb-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleEditChange("jd_link", e.target.value)
                      }
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleEditChange("resume_link", e.target.value)
                      }
          ...
          <DialogContent className="max-h-[85vh] overflow-y-auto bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 border-0 shadow-2xl">
            <div className="relative">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                Create a new meeting with all the details
              </p>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleAddFormChange("start_time", e.target.value)
                      }
                      required
                      // min="17:00"
                      // max="02:00"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleAddFormChange("end_time", e.target.value)
                      }
                      required
                      // min="17:00"
                      // max="02:00"
                    ...
                    <div className="flex flex-wrap gap-2 mb-2 min-h-[42px] border-0 bg-white/80 dark:bg-gray-700/80 rounded-lg px-3 py-2 shadow-sm">
                      {addForm.cc_members.length === 0 ? (
                        <span className="text-gray-400 text-sm self-center">
                          No CC members selected
                        </span>
                      ) : (
                        addForm.cc_members.map((m) => (
                          <span
                            key={m.id}
                            className="flex items-center gap-1 bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 rounded-full px-3 py-1 text-xs font-medium shadow-sm dark:from-pink-900/30 dark:to-purple-900/30 dark:text-pink-300"
                          >
                            {m.name}
                            <button
                              type="button"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleAddFormChange("jd_link", e.target.value)
                      }
                      placeholder="https://example.com/job-description"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleAddFormChange("resume_link", e.target.value)
                      }
                      placeholder="https://example.com/resume"
                    />
                  </div>
